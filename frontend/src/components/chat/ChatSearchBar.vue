<template>
    <button class="btn btn-ghost btn-xs" :onclick="'emaTren' + index + '.showModal()'">Add members</button>
        <dialog :id="'emaTren' + index" class="modal">
        <div class="modal-box">
            <div>
        <div  class="text-center mb-4">
            ...
            <ul v-if="searchResults.length > 0 && searchQuery">
              <li v-for="result in searchResults" :key="result.id" @click="addMember(result.id)" class="mb-1 mt-1 btn btn-sm btn-outline btn-wide btn-accent search-results">{{ result.username }}</li>
            </ul>
...
const props = defineProps({
  ...
  channel : {
    type: Object as PropType<IChannel> | undefined,
    required: true,
  },
  index: {
    type: Number,
